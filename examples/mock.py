...
Only imported when `--dry-run` is set. Wires httpx.MockTransport into
ArenaClient so the full happy path runs end-to-end with zero network access.
"""
...
def mock_table(competition_id: str) -> dict:
    """Synthetic table for dry-run: hero has AsKs on Ah Kd 7c flop, facing a bet."""
    return {
        "id": "tbl_dry", "tableId": "tbl_dry", "tableNumber": 1,
        "competitionId": competition_id, "status": "Active",
        "street": "Flop", "potChips": 300, "currentBet": 100,
        "minRaiseTo": 200, "startedAt": 1700000000000, "endedAt": None,
        "actionDeadlineAt": None, "currentSeatNumber": 1,
        "boardCards": ["Ah", "Kd", "7c"],
        "smallBlindChips": 10, "bigBlindChips": 20, "buyInChips": 2000,
        "winners": [],
        "seats": [
            {"seatId": "s1", "seatNumber": 1, "agentId": "me", "agentName": "Me",
             "agentHandle": "me", "status": "Active", "stackChips": 1800,
             "currentBetChips": 0, "totalCommittedChips": 0, "payoutChips": None,
             "holeCards": ["As", "Ks"]},
            {"seatId": "s2", "seatNumber": 2, "agentId": "opp", "agentName": "Opp",
             "agentHandle": "opp", "status": "Active", "stackChips": 1700,
             "currentBetChips": 100, "totalCommittedChips": 100, "payoutChips": None,
             "holeCards": None},
        ],
        "actingSeatNumber": 1, "selfSeatNumber": 1,
        "allowedActions": {
            "canFold": True, "canCheck": False, "canCall": True, "canBet": False,
            "canRaise": True, "callAmount": 100, "callChips": 100, "callToAmount": 100,
            "minBet": None, "minRaiseTo": 200, "maxCommit": 1800,
            "allInToAmount": 1800, "betRange": None,
            "raiseRange": {"min": 200, "max": 1800}, "canAllIn": True,
    ...
    return {
        "endpoints": [
            ...
            {"method": "POST", "path": "/api/arena/texas/benchmark/start", "auth": True,
             "output": {"properties": {"match": {"properties": {
                 "phase": {"enum": ["queued", "panel_acting", "waiting_user",
...
def run_mock_benchmark(args: argparse.Namespace,
    ...
    def _status_payload(phase: str, completed: int, table: Optional[dict]) -> dict:
        return {
            "match": {
                ...
                "phase": phase, "targetHands": target_hands, "completedHands": completed,
    ...
    def _handler(request: httpx.Request) -> httpx.Response:
        ...
        if path.endswith("/texas/action"):
            ...
            if scenario == "stale" and not stale_served["v"]:
                ...
                return httpx.Response(409, json={"error": "stale, re-poll"})
    ...
    try:
        ...
        start_resp = client.post("/texas/benchmark/start", {"competitionId": competition_id})
        ...
        from agent import _run_benchmark_loop
